...
//! userspace-random is a rust implementation of a library that attempts to securely generate
//! random numbers in userspace even if the OS entropy is compromised. The library exploits CPU
//! jitter to generate entropy - the CPU does not always take the same amount of time when
//! computing the result of a hash function.
//!
